...
const API_BASE = "https://rockystore-api.vercel.app";
const userSeenNoti = new Map();
const AUTOSYNC_CACHE_PATH = path.join(process.cwd(), "rockystore_sync_cache.json");
...
async function checkSelfUpdate() {
  ...
  try {
    const res = await axios.get(`${API_BASE}/rockystore/search?q=goatstore&limit=10&type=goat-command`);
...
async function getTodayUpdates() {
  try {
    const [c, e] = await Promise.all([
      axios.get(`${API_BASE}/rockystore/list?limit=50&type=goat-command`),
      axios.get(`${API_BASE}/rockystore/list?limit=50&type=goat-event`)
    ]);
  ...
  for (const { dir, kind } of folders) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const res = await axios.post(`${API_BASE}/rockystore/upload`, { rawUrl, framework: "goat", kind, author, category });
...
async function doInstall(api, threadID, id, forceKind = null) {
  let cmdData = null;
  try {
    const res = await axios.get(`${API_BASE}/rockystore/search?q=${encodeURIComponent(id)}`);
    const data = res.data;
  ...
  const displayName = cmdData.name || `rs_${id}`;
  ...
  try { await axios.post(`${API_BASE}/rockystore/install/${cmdData.id}`); } catch (_) {}
...
async function sendListPage(api, threadID, senderID, type, page, limit = 10) {
  ...
  try {
    const res = await axios.get(`${API_BASE}/rockystore/list?limit=${limit}&offset=${offset}&type=${type}`);
    const data = res.data;
...
async function sendSearchPage(api, threadID, senderID, query, page, limit = 5) {
  ...
  try {
    const [cr, er] = await Promise.all([
      axios.get(`${API_BASE}/rockystore/search?q=${encodeURIComponent(query)}&limit=${limit}&offset=${offset}&type=goat-command`),
      axios.get(`${API_BASE}/rockystore/search?q=${encodeURIComponent(query)}&limit=${limit}&offset=${offset}&type=goat-event`)
    ]);
...
async function uploadFile(api, threadID, filePath, kind) {
  ...
  try {
    const res = await axios.post(`${API_BASE}/rockystore/upload`, { rawUrl, framework: "goat", kind });
...
module.exports = {
  config: {
    ...
    countDown: 3,
    role: 2,
    shortDescription: "Rocky Store — Search, Install, Upload, AutoSync",
    longDescription: "Browse, install, upload, and autosync GoatBot commands and events from the Rocky Store API. Owned and maintained by Rocky Chowdhury.",
    category: "system",
    guide: {
      en:
  ...
  onStart: async function ({ api, event, args }) {
    ...
    if (!sub) {
      ...
      return api.sendMessage(
        `📦 Rocky Store — by Rocky Chowdhury\n\nUsage:\n` +
        `• !gs <id | name>\n` +
        `• !gs n\n` +
        `• !gs list [page]\n` +
        `• !gs list event [page]\n` +
    ...
    if (sub === "event") {
      ...
      if (!action) {
        try {
          const res = await axios.get(`${API_BASE}/rockystore/list?limit=20&type=goat-event`);
          const events = res.data.commands || [];
          if (!events.length) return api.sendMessage("❌ No GoatBot events found in store.", threadID);
          let msg = `📂 Rocky Store Events (${res.data.total})\n\n`;
          events.forEach(cmd => {
            msg += `╭─‣ ${cmd.name}\n├‣ ID : ${cmd.id}\n├‣ Author : ${cmd.author}\n╰────────────◊\n\n`;
          });
      ...
      try {
        const res = await axios.get(`${API_BASE}/rockystore/search?q=${encodeURIComponent(action)}&limit=5&type=goat-event`);
        const events = res.data.commands || [];
        if (!events.length) return api.sendMessage(`❌ No GoatBot event found: "${action}"`, threadID);
        let msg = `📂 Rocky Store Events matching "${action}"\n\n`;
        events.forEach(cmd => {
    ...
    if (sub === "like") {
      ...
      try {
        const res = await axios.post(`${API_BASE}/rockystore/like/${id}`, { userID: senderID });
    ...
    if (sub === "trend" || sub === "trending") {
      try {
        const res = await axios.get(`${API_BASE}/rockystore/trending?limit=5`);
        ...
        let msg = `🔥 Top Rocky Store Trending 🔥\n\n`;
        list.forEach((cmd, i) => {
          msg +=
            `╭─‣ ${cmd.name}${i === 0 ? " 🏆" : ""}\n` +
    ...
    if (sub === "delete") {
      ...
      try {
        const res = await axios.post(`${API_BASE}/rockystore/delete/${id}`, { secret });
    ...
    try {
      const res = await axios.get(`${API_BASE}/rockystore/search?q=${encodeURIComponent(query)}`);
      const data = res.data;
